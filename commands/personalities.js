...
module.exports = {
  ...
  async execute(interaction, state) {
    ...
    const hasManageMessagesPermission =
    ...
    for (let i = 0; i < state.personalities.length; i++) {
      ...
      if (
        typeof process.env["description." + thisPersonality.name] !==
        "undefined"
      ) {
        // Truncate the description to 1024 characters if it's longer than that
        let truncatedPrompt = process.env[
          "description." + thisPersonality.name
        ].substring(0, 1024);
        persEmbed.addFields({
          name: thisPersonality.name,
          value: truncatedPrompt,
        });
      } else {
        // Find the prompt from the request array
        let prompt = thisPersonality.request.find(
