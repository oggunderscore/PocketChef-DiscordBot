...
client.on("ready", () => {
  ...
  console.log("[DEBUG] Bot is ready and connected to Discord.");
});
...
function getPersonality(message) {
  ...
  console.log("[DEBUG] Checking personality for message:", message);
...
client.on("messageCreate", async (msg) => {
  console.log("[DEBUG] Message received:", msg.content);
  ...
  if (p == null) {
    console.log("[DEBUG] No matching personality found for message.");
    return;
  }
  ...
  if (
    ...
    p.request.length - 1 > parseInt(process.env.MSG_LIMIT, 10)
  ) {
...
async function chat(requestX, msg) {
  try {
    // Make API request
    console.log("[DEBUG] API request sent:", requestX);
    ...
    console.log("[DEBUG] API response received:", responseContent);
